...
// --- Type Definitions ---
interface PropSchema { name: string; type: string; options: string[]; defaultValue: string; description: string; }
interface Component { name: string; dependencies: string[]; files: string[]; usage?: string; props: Record<string, PropSchema>; }
interface Config { registryUrl: string; remoteBaseUrl: string; targetDir: string; libDir: string; }
type Registry = Record<string, Component>;

// --- Helper: Load Config ---
async function loadConfig(quiet = false): Promise<Config | null> {
...
// --- Command: show ---
async function showCommand(componentName: string) {
  try {
    ...
    const component = registry[componentName] as Component;
...
// --- Command: add ---
async function addComponent(componentName: string, options: { dryRun?: boolean }) {
  try {
    const config = await loadConfig();
    if (!config) return;
...
// --- Command: connect ---
async function connectCommand(options: { url?: string }) {
  try {
